...
if(con == 2 && !instance_exists(782/* OBJ_WRITER */)) {
    ...
    draw_sprite(49/* spr_heartsmall */, 0, __view_get( e__VW.XView, 0 ) + 30 + heartx * 110, __view_get( e__VW.YView, 0 ) + 20 + 20 * hearty);
    if(buffer < 0) {
        if(keyboard_check_pressed(38/* UP */) && hearty > 0)
            hearty--;
        if(keyboard_check_pressed(40/* DOWN */) && hearty < 2) {
            if(heartx == 0) {
                if(hearty == 1) hearty++;
                if(hearty == 0) hearty++;
            }
        ...
        if(keyboard_check_pressed(39/* RIGHT */) && heartx == 0) {
            if(hearty == 0) heartx++;
            if(hearty == 1) heartx++;
            if(hearty == 2) heartx++;
        }
        if(keyboard_check_pressed(37/* LEFT */) && heartx == 1) {
            if(hearty == 0) heartx--;
            if(hearty == 1) heartx--;
            if(hearty == 2) heartx--;
        }
